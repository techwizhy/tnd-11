...
function initPopout3dCardTilt() {
  ...
  cards.forEach(card => {
    ...
    card.addEventListener('mousemove', (e) => {
      ...
      // Tilt the entire container card in 3D perspective
      card.style.transform = `perspective(1000px) rotateY(${rotateY}deg) rotateX(${rotateX}deg)`;
      card.style.transition = 'transform 0.1s ease-out';
      
      // Apply the golden glow background light to the bg box
      if (bg) {
      ...
      // Apply subtle shadow and border to bottom text box
      if (info) {
    ...
    card.addEventListener('mouseleave', () => {
      ...
      if (bg) {
        bg.style.boxShadow = '0 15px 35px rgba(0, 0, 0, 0.1)';
        bg.style.borderColor = 'rgba(255, 255, 255, 0.7)';
        bg.style.transition = 'box-shadow 0.8s cubic-bezier(0.16, 1, 0.3, 1), border-color 0.8s ease';
      }
      
      if (info) {
        info.style.boxShadow = '0 10px 25px rgba(0, 0, 0, 0.05)';
...
var ParticleSystem = (() => {
  ...
  function loop() {
    if (!isHeroVisible || document.hidden) {
      animFrameId = null;
      return; // Stop the loop when hero is offscreen or tab is hidden
    }
  ...
  function initialize() {
    ...
    // Intersection Observer & Page Visibility API: pause/resume WebGL loop
    ...
    if (heroSection && 'IntersectionObserver' in window) {
      const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          ...
          if (isHeroVisible && !document.hidden && !animFrameId) {
            animFrameId = requestAnimationFrame(loop);
          }
        });
      }, { threshold: 0 });
    ...
    document.addEventListener('visibilitychange', () => {
      if (!document.hidden && isHeroVisible && !animFrameId) {
        animFrameId = requestAnimationFrame(loop);
      }
    });
  }
...
function initHeroMotionsites() {
  ...
  setTimeout(() => {
    ...
    heroBody && heroBody.classList.add('show');
  }, 400);
  ...
  // 3. Legacy Flip Card (no-op – element may not be .flip-scene anymore)
  ...
  if (card) {
    ...
    if (scene) {
      ...
      scene.addEventListener('touchend', () => { setTimeout(play, 1500); });
    }
  }
